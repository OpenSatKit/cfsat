"""
Core Flight Executive (cFE) Electronic Data Sheet (EDS) database interface classes. EDSs are used to define an
application's interface.  The cfe-eds build toolchain produces artifacts that can be used by flight and ground
software that needs to interface with an application. The following python binding files are created:

    EdsLib        - Contains generic EDS structure definitions
    CFE_MissioLib - Contains mission-specific definitions. The top-level structure
                    is a list of application 'instances'

The following environment variables must be defined to use the EDS python libraries:

    LD_LIBRARY_PATH - TODO
    PYTHONPATH      - TODO

The EDS and the python classes in this file use the following concepts/definitions:

    Mission     - The name of a mission that can include mutliple cFS deployments
    cFS Target  - A single cFS deployment on a processor. The EDS supports application
                  "instances" and in theory multiple instances could be deployed on a
                  single cFS target. However the current cFE release Caelum does not
                  support this model.
